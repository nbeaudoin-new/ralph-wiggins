...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "scrape",
            description: "Navigate to a URL and return the page title + cleaned text content in one step.",
            inputSchema: {
                type: "object",
                properties: {
    ...
    try {
        switch (name) {
            ...
            case "navigate": {
                ...
                const waitUntil = a.wait_until ?? "load";
                await p.goto(a.url, { waitUntil, timeout: 30_000 });
            ...
            case "scrape": {
                const p = await getPage();
                await p.goto(a.url, { waitUntil: "domcontentloaded", timeout: 30_000 });
                if (a.wait_for_selector) {
                    await p.waitForSelector(a.wait_for_selector, { timeout: 10_000 });
                }
                const title = await p.title();
                const url = p.url();
            ...
            case "get_text": {
                ...
                let text;
                if (a.selector) {
                    const el = await p.locator(a.selector).first();
            ...
            case "screenshot": {
                ...
                let buffer;
                if (a.selector) {
                    const el = p.locator(a.selector).first();
            ...
            case "click": {
                const p = await getPage();
                const timeout = a.timeout ?? 5_000;
                await p.click(a.selector, { timeout });
            ...
            case "fill": {
                const p = await getPage();
                await p.fill(a.selector, a.value);
                return { content: [{ type: "text", text: `Filled "${a.selector}" with value.` }] };
            }
            // ── select ────────────────────────────────────────────────────────────
            case "select": {
                const p = await getPage();
                await p.selectOption(a.selector, { label: a.value });
            ...
            case "wait_for": {
                ...
                await p.waitForSelector(a.selector, { state, timeout });
                return { content: [{ type: "text", text: `Element "${a.selector}" is ${state}.` }] };
            }
            // ── evaluate ──────────────────────────────────────────────────────────
            case "evaluate": {
                const p = await getPage();
                const result = await p.evaluate(a.script);
                return {
            ...
            case "scroll": {
                const p = await getPage();
                if (a.selector) {
                    await p.locator(a.selector).first().scrollIntoViewIfNeeded();
                }
                else {
                    await p.evaluate(({ x, y }) => window.scrollTo(x, y), { x: a.x ?? 0, y: a.y ?? 0 });
                }
